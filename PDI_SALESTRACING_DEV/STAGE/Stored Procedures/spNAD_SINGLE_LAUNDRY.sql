Create PROCEDURE [STAGE].[spNAD_SINGLE_LAUNDRY] 
@ID INT = NULL
WITH EXEC AS CALLER
AS
BEGIN  
...
-- Remove all extra spaces from the fields
  ...
  UPDATE STAGE.TEMP_ADDR_LAUNDRY with(tablock)
    SET 
      ...
      ADDR1_STAT_CD = 9 -- meaning in progress
...
-- Location 108
 ...
 ------ Break Apart ADDR1 -- 
  ...
  -- Grab street name               -- 6 sec
  UPDATE STAGE.TEMP_ADDR_LAUNDRY 
    SET ST_1 = CASE WHEN Len(ST_NR_1)>0
                    THEN STAGE.fnSecondPart(ADDR1,Len(ST_NR_1)+1)
                    ELSE ADDR1 END
  WHERE SRCE_DATA_ID = ISNULL(@ID ,SRCE_DATA_ID );   
 ...
 -------------------------------------------  ADDR LINE 2 START ----------------------------
  ...
  -- First round - addr2 stats with SUITE, BLDG ETC. when Addr2 has npo value
    ...
    WHERE  
      ...
      AND T.ADRR2_TYP IS NULL
      --AND T.SRCE_DATA_ID = ISNULL(@ID ,SRCE_DATA_ID 
      OPTION (MERGE JOIN);
...
--------------------------------------    
  ...
  SELECT COUNT(*)
  FROM STAGE.TEMP_ADDR_LAUNDRY
  WHERE UPD_ADDR1 IS NULL
  
  ALTER TABLE STAGE.TEMP_ADDR_LAUNDRY
  ADD ISSUE_CD VARCHAR(20);
--------------------------------------
  ...
  UPDATE STAGE.TEMP_ADDR_LAUNDRY 
    SET UPD_ADDR1 = 
          ...
          +' '+
          CASE WHEN ST_NM_1 IS NOT NULL THEN ST_NM_1 ELSE '' END
          +' '+
  ...
  UPDATE STAGE.TEMP_ADDR_LAUNDRY 
    SET UPD_ADDR1 = NULL
  WHERE LEN(UPD_ADDR1) = 0;

  
  -- UPD_ADDR1 WHICH IS LEFT --
  
  UPDATE T
      SET UPD_ADDR1 = REPLACE(ST_1,A.ADDR_VAR, A.ADDR_STD)
  FROM STAGE.TEMP_ADDR_LAUNDRY T,
       REF.ADDR_STD A
  WHERE 
    UPD_ADDR1 IS NULL 
  ...
  -- Remains unclean - should flag this -- 
    UPDATE STAGE.TEMP_ADDR_LAUNDRY 
    SET UPD_ADDR1 = ST_1,ISSUE_CD = 'Unclean_Addr'  
    WHERE UPD_ADDR1 IS NULL ;
