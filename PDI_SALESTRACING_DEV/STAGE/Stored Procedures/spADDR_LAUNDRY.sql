CREATE PROCEDURE [STAGE].[spADDR_LAUNDRY]
...
/*
    ...
    */
    ...
    UPDATE MDM_STAGE.TEMP_ADDR SET 
      ZIP = CASE WHEN LEN(ZIP)>4 THEN LEFT(ZIP,5) ELSE '0'+ZIP END;
    
    UPDATE MDM_STAGE.TEMP_ADDR SET WB_2 =NULL WHERE LEN(WB_2) = 0;
  ...
  ----------------------------    S T A G E   1   ------------------------------------
    ...
    UPDATE MDM_STAGE.TEMP_ADDR SET STE_NR = STAGE.fnRemoveSpace(REPLACE(STE_NR,'#',' '))
    WHERE STE_NR LIKE '%#%';
    
    UPDATE MDM_STAGE.TEMP_ADDR SET STE_NR = rtrim(STAGE.fnGet_FirstPart(STE_NR, charindex('-',STE_NR)-1))
    WHERE STE_NR LIKE '%-%';
    
    UPDATE MDM_STAGE.TEMP_ADDR SET BLDG_NR = STAGE.fnRemoveSpace(REPLACE(BLDG_NR,'#',' ')) 
    WHERE BLDG_NR LIKE '%#%';
    
    UPDATE MDM_STAGE.TEMP_ADDR SET BLDG_NR = rtrim(STAGE.fnGet_FirstPart(BLDG_NR, charindex('-',BLDG_NR)-1))
    WHERE BLDG_NR LIKE '%-%'; 
    
    UPDATE MDM_STAGE.TEMP_ADDR SET STE_NR = NULL
    WHERE LEN(STE_NR) > 9;
    
    -- Get the street part
    UPDATE MDM_STAGE.TEMP_ADDR SET
      ST_NR = STAGE.fnFirstWord(WB_1),
      ST_NM = STAGE.fnMiddlePart(WB_1,STAGE.fnFirstWord(WB_1),STAGE.fnLastWord(WB_1) ),
      ST_TYP = STAGE.fnLastWord(WB_1) 
    WHERE STAT_1_CD = 1 AND STAT_2_CD IN (-1,1);
    
    -- IF THERE IS DIRECTION IN THE NAME - then it needs to be taken out
    UPDATE  C SET
      DIR_1 = A.ADDR_STD, 
      ST_NM = STAGE.fnSecondPart(ST_NM,LEN(STAGE.fnFirstWord(ST_NM))+1)
    FROM MDM_STAGE.TEMP_ADDR C
    JOIN REF.ADDR_STD A ON STAGE.fnFirstWord(ST_NM) = A.ADDR_VAR AND A.CAT = 'DIRECTION' 
    WHERE STAGE.fnWordCount(ST_NM) > 1  -- IF THE NAME OS THE STREET IS SOUTH - WE ARE KEEPING IT THAT WAY
    AND STAT_1_CD = 1 AND STAT_2_CD IN (-1,1)
    OPTION (MERGE JOIN);
    
    -- GET RID OF this big chunk of records
   TRUNCATE TABLE MDM_STAGE.TEMP_ADDR_PARTS;
    
  INSERT INTO MDM_STAGE.TEMP_ADDR_PARTS (  
    SRC_ID, ST_NR, DIR_1, ST_NM,  ST_TYP, ST_DIR, BLDG_NR, FL_NR, STE_NR )
  SELECT 
    SRC_ID, ST_NR, DIR_1, ST_NM, ST_TYP , DIR_2, BLDG_NR, FL_NR, STE_NR
...
-- Take care of ADDR 2
  ...
  INSERT INTO MDM_STAGE.TEMP_ADDR_PARTS (  
    SRC_ID, ST_NR, ST_NR_2, DIR_1,  ST_NM,  ST_TYP, ST_DIR, BLDG_NR, FL_NR, STE_NR )
  SELECT 
    SRC_ID, ST_NR, ST_NR_2, DIR_1, ST_NM, ST_TYP ,DIR_2, BLDG_NR, FL_NR, STE_NR
  FROM MDM_STAGE.TEMP_ADDR
  WHERE STAT_1_CD = 1 AND STAT_2_CD IN (-1,1)
  ...
  SELECT 
    ...
    -- DO SOME FIXING
    --------------------
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET
      ST_NR = LTRIM(RTRIM(ST_NR)),
      ST_NR_2 = CASE WHEN LEN(ST_NR_2) =0 THEN NULL ELSE LTRIM(RTRIM(ST_NR_2)) END,
      DIR_1 = CASE WHEN LEN(DIR_1) =0 THEN NULL ELSE LTRIM(RTRIM(DIR_1)) END,
      ST_NM = LTRIM(RTRIM(STAGE.fnRemovespace(ST_NM))),
      ST_DIR = CASE WHEN LEN(ST_DIR) =0 THEN NULL ELSE LTRIM(RTRIM(ST_DIR)) END, 
      BLDG_NR = CASE WHEN LEN(BLDG_NR) =0 THEN NULL ELSE LTRIM(RTRIM(BLDG_NR)) END, 
      FL_NR = CASE WHEN LEN(FL_NR) =0 THEN NULL ELSE LTRIM(RTRIM(FL_NR)) END, 
      STE_NR = CASE WHEN LEN(STE_NR) =0 THEN NULL ELSE LTRIM(RTRIM(STE_NR)) END;
    
    
    UPDATE  C 
      SET ST_TYP = A.ADDR_STD,
      ST_NM = STAGE.fnGet_FirstPart(ST_NM, LEN(ST_NM)-LEN(A.ADDR_STD)-1)
    FROM MDM_STAGE.TEMP_ADDR_PARTS C
    JOIN ( SELECT DISTINCT ADDR_STD FROM REF.ADDR_STD WHERE CAT IN ('Roadway','Location')) A 
        ON STAGE.fnLastWord(ST_NM) = A.ADDR_STD 
    WHERE ST_TYP IS NULL AND STAGE.fnWordCount(ST_NM) > 1
    OPTION (MERGE JOIN) ;
    
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'EAST' WHERE ST_NM = 'E';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'WEST' WHERE ST_NM = 'W';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'NORTH' WHERE ST_NM = 'N';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'SOUTH' WHERE ST_NM = 'S';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'NORTHWEST' WHERE ST_NM = 'NW';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'NORTHEAST' WHERE ST_NM = 'NE';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'SOTHWEST' WHERE ST_NM = 'SW';
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET ST_NM = 'SOUTHEAST' WHERE ST_NM = 'SE';
    
    -------------
    -- FIX STE_NR
    -------------
    -- Direction Issues -- THIS ONE SHOULD BE FIXED BY CORRECTING THE CODE -- 
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET 
      ST_DIR = STE_NR, -- CASE WHEN ST_DR IS NULL THEN STE_NR ELSE ST_DR END, 
      STE_NR = NULL
    WHERE LTRIM(RTRIM(STE_NR)) IN ('N','S','W','SE','SW','NW','NE');  
    
    -- IF FIRST LETTER IS A STRING THEN IT SHOULD BE A - Z 
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET STE_NR = NULL
    WHERE ISNUMERIC(LEFT(STE_NR,1)) = 0 AND LEN(STE_NR)>1; 
    
    -- IF FIRST LETTER IS A NUMERIC THEN IT SHOULD NOT BE MORE THAN 5 CHARCATER - 1000A 
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET STE_NR = NULL
    WHERE ISNUMERIC(LEFT(STE_NR,1)) = 1 AND LEN(STE_NR)>5; 
    
    ----------------
    -- FIX FLOOR ---
    ----------------
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET FL_NR = NULL
    WHERE ISNUMERIC(FL_NR) = 0; 
    ...
    -- Direction Issues -- THIS ONE SHOULD BE FIXED BY CORRECTING THE CODE -- 
   /* UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET 
      ST_DIR = BLDG_NR, -- CASE WHEN ST_DR IS NULL THEN BLDG_NR ELSE ST_DR END, 
      BLDG_NR = NULL
    WHERE LTRIM(RTRIM(BLDG_NR)) IN ('SE','SW','NW','NE');  */
    
    -- IF FIRST LETTER IS A STRING THEN IT SHOULD BE A - Z 
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET BLDG_NR = NULL
    WHERE ISNUMERIC(LEFT(BLDG_NR,1)) = 0 AND LEN(BLDG_NR)>2; 
    
    -- IF FIRST LETTER IS A NUMERIC THEN IT SHOULD NOT BE MORE THAN 5 CHARCATER - 1000A 
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET BLDG_NR = NULL
    WHERE ISNUMERIC(LEFT(BLDG_NR,1)) = 1 AND LEN(BLDG_NR)>5; 
    
  ------------------------------------
  --  INSERT CORRECT ADDRESS BACK ----
  ------------------------------------  

    
    UPDATE MDM_STAGE.TEMP_ADDR_PARTS SET 
      ADDR_1 = STAGE.fnRemoveSpace( ISNULL(ST_NR,'')+ISNULL(ST_NR_2,'')+' '+ISNULL(DIR_1,'')+' '+ISNULL(ST_NM,' ')+' '+ISNULL(ST_TYP,'')+' '+ISNULL(ST_DIR,'')),
      ADDR_2 =  STAGE.fnRemoveSpace( (CASE WHEN BLDG_NR IS NOT NULL THEN 'BLDG '+BLDG_NR ELSE '' END)+' '+(CASE WHEN FL_NR IS NOT NULL THEN 'FL '+FL_NR ELSE '' END)+' '
    +(CASE WHEN STE_NR IS NOT NULL THEN 'STE '+STE_NR ELSE '' END));
 END;