...
BEGIN
  ...
  ****************************/
  ...
  ------  UPDATE ADDRESS  -------
    UPDATE C SET
      C.BUYER_GRP_ADDR_1 = CASE WHEN A.ADDR_1 LIKE 'NUL%' THEN NULL ELSE A.ADDR_1 END, 
      C.BUYER_GRP_CITY = CASE WHEN A.CITY LIKE 'NUL%' THEN NULL ELSE A.CITY END, 
      C.BUYER_GRP_ST = CASE WHEN A.STATE LIKE 'NUL%' THEN NULL ELSE A.STATE END , 
      C.BUYER_GRP_ZIP = CASE WHEN A.ZIP LIKE 'NUL%' THEN NULL ELSE A.ZIP END 
    FROM CNT.[CONTRACT] C
    JOIN STAGE.CONTRAXX_CNT_ADDR A 
      ON C.CNT_NR = A.CNT_NR AND C.REC_STAT_CD = 'A'
    WHERE LEN(A.STATE)>0 OR  A.STATE NOT LIKE 'NU%';
