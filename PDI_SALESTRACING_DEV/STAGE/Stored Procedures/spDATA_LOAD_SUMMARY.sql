CREATE PROCEDURE [STAGE].[spDATA_LOAD_SUMMARY]
...
*/
  ...
  TRUNCATE TABLE STAGE.DATA_LOAD_SUMMARY ;
 
  -----------------------------
  -- LOAD THE SUMMARY TABLE --
  -----------------------------
  
  INSERT INTO STAGE.DATA_LOAD_SUMMARY 
  ...
  UPDATE STAGE.DATA_LOAD_SUMMARY 
  SET SALES_PERIOD_DT = 
  ...
  WHILE @@FETCH_STATUS = 0
    BEGIN
      SELECT @SALES_6MO = AVG(SALES_AMT)  FROM STAGE.DATA_LOAD_SUMMARY 
      WHERE DIST_NR = @DIST_NR 
      AND SALESPERIOD BETWEEN DATEADD(M,-6,@SALES_PRIOD_DT) AND DATEADD(M,-1,@SALES_PRIOD_DT);
      
      SELECT @SALES_12MO = AVG(SALES_AMT)  FROM STAGE.DATA_LOAD_SUMMARY 
      WHERE DIST_NR = @DIST_NR 
      AND SALESPERIOD BETWEEN DATEADD(M,-12,@SALES_PRIOD_DT) AND DATEADD(M,-1,@SALES_PRIOD_DT);
      
      SELECT @QTY_6MO = AVG(CS_QTY)  FROM STAGE.DATA_LOAD_SUMMARY 
      WHERE DIST_NR = @DIST_NR 
      AND SALESPERIOD BETWEEN DATEADD(M,-6,@SALES_PRIOD_DT) AND DATEADD(M,-1,@SALES_PRIOD_DT);
      
      SELECT @QTY_12MO = AVG(CS_QTY)  FROM STAGE.DATA_LOAD_SUMMARY 
      WHERE DIST_NR = @DIST_NR 
      AND SALESPERIOD BETWEEN DATEADD(M,-12,@SALES_PRIOD_DT) AND DATEADD(M,-1,@SALES_PRIOD_DT);
      ...
      SET CS_QTY_6MO_AVG = @QTY_6MO, 
         ...
         /* SALESPERIOD =     
            YEAR(DATEADD(M,1,@SALES_PRIOD_DT))*10000
            +
            MONTH(DATEADD(M,1,@SALES_PRIOD_DT))*100
            +
            DAY(DATEADD(M,1,@SALES_PRIOD_DT))*/
