CREATE PROCEDURE [STAGE].[spCONTRACT_MASTER_v2]
WITH EXEC AS CALLER
AS
/*
...
UPDATE CNT.[CONTRACT] SET BUYER_GRP_NM = 'INTALERE' WHERE BUYER_GRP_NM LIKE '%AMERINET%';
UPDATE CNT.[CONTRACT] SET BUYER_GRP_NM = 'VIZIENT' WHERE BUYER_GRP_NM LIKE '%MEDASSETS%';
UPDATE CNT.[CONTRACT] SET BUYER_GRP_NM = 'VIZIENT' WHERE BUYER_GRP_NM LIKE '%NOVATION%';
UPDATE CNT.[CONTRACT] SET BUYER_GRP_NM = 'VIZIENT' WHERE BUYER_GRP_NM LIKE '%VIZIENT%';
UPDATE CNT.[CONTRACT] SET BUYER_GRP_NM = 'ROI' WHERE (BUYER_GRP_NM LIKE 'ROI/%' OR BUYER_GRP_NM LIKE 'ROI-%' OR BUYER_GRP_NM LIKE 'ROI %');  

UPDATE C SET C.BUYER_GRP_ID = G.PDI_GRP_ID
from [STAGE].[PRCHS_GRP] G
JOIN CNT.[CONTRACT] C ON LTRIM(RTRIM(G.CMS_NM)) = LTRIM(RTRIM(C.BUYER_GRP_NM))
  ...
  ****************************/
  ...
  IF @REC_CNT > 0
    ...
    END;  
       ...
       -- KEEPING THE CONTRACT INTACT - THIS SHOULD ONLY BE IN PRODUCT TABLE
   /* 
    SET @REC_CNT = 0;      
    SELECT @REC_CNT = COUNT(*) 
