CREATE PROCEDURE [STAGE].[spADDR_LAUNDRY_X] (
@SrcTbl varchar(100), @SrcTbl_id varchar(50), @SrcAddr1 varchar(50), @SrcAddr2 varchar(50), @SrcZip varchar(50),
@DesTbl varchar(100), @DesTbl_id varchar(50), @DesAddr1 varchar(50), @DesAddr2 varchar(50)--, @DesZip varchar(20)
)
...
BEGIN  
 ...
 ------------- FIRST LOAD THE DATA ----------
 SET @SQLSTR =
 'INSERT INTO MDM_STAGE.TEMP_ADDR (SRC_ID, ADDR_1, ADDR_2, ZIP )
 SELECT '+@SrcTbl_id+', '+@SrcAddr1+', '+@SrcAddr2+', '+@SrcZip+
 ' FROM '+@SrcTbl ;
...
-- Take care of ADDR 2
  ...
  SET @SQLSTR =
    'UPDATE T SET '+
    @DesAddr1+' = ISNULL(ST_NR,'''')+ISNULL(ST_NR_2,'''')+'' ''+ISNULL(DIR_1,'''')+'' ''+ST_NM+'' ''+ISNULL(ST_TYP,'''')+'' ''+ISNULL(ST_DIR,''''), '+
    @DesAddr2+' = STAGE.fnRemoveSpace( (CASE WHEN BLDG_NR IS NOT NULL THEN ''BLDG ''+BLDG_NR ELSE '''' END)+'' ''+(CASE WHEN FL_NR IS NOT NULL THEN ''FL ''+FL_NR ELSE '''' END)+'' ''
    +(CASE WHEN STE_NR IS NOT NULL THEN ''STE ''+STE_NR ELSE '''' END)) FROM '+@DesTbl+' T JOIN MDM_STAGE.TEMP_ADDR_PARTS S ON S.SRC_ID = T.'+@DesTbl_id
 ...
 -- TRUNCATE TABLE MDM_STAGE.TEMP_ADDR 
        ...
        /*
    SELECT TOP 100  *,
--    A.ST_NR, A.ST_NR_2, A.DIR_1, A.ST_NM, A.ST_TYP, A.ST_DIR,
    ISNULL(ST_NR,'')+ISNULL(ST_NR_2,'')+' '+ISNULL(DIR_1,'')+' '+ST_NM+' '+ISNULL(ST_TYP,'')+' '+ISNULL(ST_DIR,''),
    STAGE.fnRemoveSpace( (CASE WHEN BLDG_NR IS NOT NULL THEN 'BLDG '+BLDG_NR ELSE '' END)+' '+(CASE WHEN FL_NR IS NOT NULL THEN 'FL '+FL_NR ELSE '' END)+' '+(CASE WHEN STE_NR IS NOT NULL THEN 'STE '+STE_NR ELSE '' END))
    FROM MDM_STAGE.CMS_ADDR_PARTS A
    WHERE A.STE_NR IS NOT NULL AND A.BLDG_NR IS NOT NULL AND A.FL_NR IS NOT NULL */
