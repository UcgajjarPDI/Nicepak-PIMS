...
BEGIN
 BEGIN
   MERGE [PDI_SALESTRACING_DEV].[STAGE].[SLS_TRC_LOAD_DTL] T
    USING (
     ...
     convert(int,replace(CONVERT(DATE, left(H.[current timestamp],10)),'-','')) AS RECVD_DTE,
      ...
      FROM  
      [PDI_SALESTRACING_DEV].FTPOUT.E867HDR_DAILY_HIST H
      JOIN [PDI_SALESTRACING_DEV].FTPOUT.E867DTL_DAILY_HIST D ON H.HDRSEQ = D.DTLSEQ AND H.HDRCUS = D.DTLCUS
      JOIN [PDI_SALESTRACING_DEV].FTPOUT.E867SDTL_DAILY_HIST S ON D.DDLSEQ = S.SDLSEQ AND D.DTLCUS = S.SDTCUS
      JOIN [PDI_SALESTRACING_DEV].STAGE.DIM_DISTRIBUTOR DI ON H.HDRCUS = DI.TRACING_NAME
      WHERE  
   ...
   UPDATE  [PDI_SALESTRACING_DEV].[STAGE].[SLS_TRC_LOAD_DTL]
  ...
  INSERT INTO [STAGE].[SALES_TRACING_CURR]
    ...
    JOIN [PDI_SALESTRACING_dev].[STAGE].[SLS_TRC_LOAD_DTL] C 
