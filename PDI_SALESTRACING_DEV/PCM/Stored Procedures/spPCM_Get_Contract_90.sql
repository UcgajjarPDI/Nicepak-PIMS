CREATE PROCEDURE [PCM].[spPCM_Get_Contract_90] 
( @UNM VARCHAR(20))
WITH EXEC AS CALLER
AS
BEGIN
 ...
 SELECT CNT_NR, 
      ...
      FROM [CNT_DEV].[CONTRACT] C
     	    where convert(date,CNT_EXP_DT)<=convert(date,DATEADD(day,90,getdate()),101)
and c.BUYER_GRP_NM is null
...
--      JOIN PCM.PCM_GROUP G ON C.PDI_GROUP_ID = G.PDI_GRP_ID
--      WHERE U.USR_ID = @UNM and convert(date,CNT_EXP_DT)<=convert(date,DATEADD(day,30,getdate()),101)
--    END
--  END
--ELSE
--  BEGIN
