CREATE PROCEDURE [TRC].[spTRC_GET_EXC_CORR_by_grp]
@vCNT_ID varchar(30) = null, @vPROD_ID varchar(15) = null
WITH EXEC AS CALLER
AS
BEGIN

  SELECT DISTINCT CN.GROUP_NAME, CN.CONTRACT_NO, C.ITEMID, C.[Exp Date] AS EXP_DT, 'Description' AS CNT_DES
  FROM 
  ...
  AND CO.CURRENT_INDICATOR = 'Y')
    AND (CN.GROUP_NAME = CO.GROUP_NAME AND CN.CURRENT_INDICATOR = 'Y')
